...
public interface IAzureStorageBlockBlobUploader
{
    ...
    /// <see cref="ContentUploadOptions.BlockSizeBytes"/>-sized chunks. Before staging each block, if the
    /// time remaining before <paramref name="expiresAt"/> has dropped below
    /// <see cref="ContentUploadOptions.RenewalSafetyMargin"/>, calls <paramref name="renewAsync"/> (which
    /// is expected to call the Graph <c>renewUpload</c> action and re-read the file's SAS URI) and
    /// continues with the refreshed URI/expiry.
    /// </summary>
    Task UploadAsync(
        Uri sasUri,
