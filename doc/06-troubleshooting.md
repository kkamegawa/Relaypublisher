...
- **The XAR/XML parser fails**: treat a truncated archive, invalid offsets, a compression method outside `none`/`gzip`/`bzip2`, malformed XML, external entity/DTD usage, or an extraction limit as a hard error. Obtain a complete package from the source and rerun `package`; `--force` is intentionally ineffective. If the error text is exactly `uses unsupported compression 'application/x-bzip2'`, the CLI predates bzip2 support (issue #127) — pin the current CLI version and rerun; that specific compression method is no longer a hard error.
