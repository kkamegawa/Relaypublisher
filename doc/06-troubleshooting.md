...
| Windows file detection does not validate or always fails | `Path` was treated as a repository path, an unsupported operation/operator was used, or the version value is malformed | Use only `exists` or `version`; set a target-device drive-rooted, root-relative, UNC, or environment-variable path; use a one-to-four-part numeric `ComparisonValue` for `version` |
