...
| A second app showed up after switching a macOS entry from an explicit `Architecture` to omitted (or vice versa) | Identity changed (e.g. `...\|macos\|arm64` -> `...\|macos\|universal`, issue #122) *and* `DisplayName` was also changed in the same publish, so the `DisplayName` fallback could not adopt the existing app (doc/05-operation.md §4b.2) | Keep `DisplayName` unchanged when only switching how `Architecture` is declared; the fallback then adopts the existing app instead of creating a new one. If a duplicate was already created, remove it manually in the Intune admin center |
...
- **The XAR/XML parser fails**: treat a truncated archive, invalid offsets, a compression method outside `none`/`gzip`/`bzip2`, malformed XML, external entity/DTD usage, or an extraction limit as a hard error. Obtain a complete package from the source and rerun `package`; `--force` is intentionally ineffective. Three specific error texts point at CLI version rather than a genuinely bad package (all fixed by issue #127 — pin the current CLI version and rerun):
  - `uses unsupported compression 'application/x-bzip2'` — the CLI predates bzip2 support.
  - `The archive entry was compressed using an unsupported compression method` (from a `.NET GZipStream`/`Inflater` call stack, wrapped as `... could not be read.`) — the CLI predates the fix that reads xar's `application/x-gzip` heap entries as raw zlib instead of the GZIP file format; every real `gzip`-encoded entry failed with this text on older CLI versions.
  - `A Distribution bundle entry has no bundle identifier.` — the CLI predates support for the `productbuild` `<pkg-ref><bundle-version><bundle .../></bundle-version></pkg-ref>` shape, where `<bundle-version>` is an attribute-less wrapper around the real `<bundle>` records.
