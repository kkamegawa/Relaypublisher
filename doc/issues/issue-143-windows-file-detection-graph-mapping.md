...
- Map `ruleType`, `path`, `fileOrFolderName`, `check32BitOn64System`, `operationType`, `operator`, and `comparisonValue`. Synthesize `operator: notConfigured` with a null `comparisonValue` for `operationType: exists`.
